...
public class OpenPagesUtil {
    ...
    public static List<Id> processQuery(final IQueryService queryService, final String querySelect) {
        ...
        return StreamUtil.asStream(resultSet.iterator())
                .flatMap((IResultSetRow row) -> StreamUtil.asStream(row.iterator()))
    ...
    public static Optional<IEnumValue> findMatchingEnumValue(IEnumField field, String value) {
        ...
        final Optional<IEnumValue> result = StreamUtil.asStream(field.getFieldDefinition().getEnumValues().iterator())
    ...
    public static Double calculateMetricValue(IResourceService service, IGRCObject metric) {
        final Comparator<String> c = new ReverseComparator<String>(new StringComparator());

        final Optional<IGRCObject> metricValue = metric.getChildren().stream()
                .filter(OpenPagesUtil::isMetricValue)
                .map((IAssociationNode node) -> OpenPagesUtil.lookupResource(service, node.getId()))
                .sorted((IGRCObject a, IGRCObject b) -> c.compare(getStringFieldValue(a, MetricValue.Name), getStringFieldValue(b, MetricValue.Name)))
                .findFirst();

        if (metricValue.isPresent()) {
            logger.info("  Metric value found: " + getStringFieldValue(metricValue.get(), MetricValue.Name));
        } else {
            logger.info("  Metric value not found");
        }

        return metricValue
                .flatMap(val -> getFloatFieldValue(val, MetricValue.Value))
                .map(v -> {
                    logger.info("  MetricValue(" + getStringFieldValue(metric, Metric.Name) + "): " + v);
                    return v;
                })
                .orElseGet(() -> {
                    logger.info("  Unable to retrieve MetricValue");
                    return null;
                });
    }
